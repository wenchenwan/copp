"""
TOPP3-LP results: a(s), b(s)=s̈, and s(t) for SCP iterations 1 and 2.
Run from the project root:  python scripts/plot_topp3_lp.py
"""
...
d1 = pd.read_csv("output/data/topp3_lp_iter1.csv")
d2 = pd.read_csv("output/data/topp3_lp_iter2.csv")
t1 = pd.read_csv("output/data/topp3_lp_iter1_traj.csv")
t2 = pd.read_csv("output/data/topp3_lp_iter2_traj.csv")

s      = d1["s"].values
a1, b1 = d1["a"].values, d1["b"].values
a2, b2 = d2["a"].values, d2["b"].values
t_s1, t_s2 = d1["t_s"].values, d2["t_s"].values
tf1, tf2 = t_s1[-1], t_s2[-1]

fig, axes = plt.subplots(2, 2, figsize=(13, 9))
fig.suptitle(
    f"TOPP3-LP  (SCP iterations)\n"
    f"t_final:  iter1 = {tf1:.4f} s   →   iter2 = {tf2:.4f} s  (improvement = {tf1-tf2:.4f} s)",
...
# (0,1) path speed ṡ(s)
ax = axes[0, 1]
ax.plot(s, np.sqrt(a1), color=C1, lw=1.8, label=f"Iter 1")
ax.plot(s, np.sqrt(a2), color=C2, lw=1.8, linestyle="--", label=f"Iter 2")
