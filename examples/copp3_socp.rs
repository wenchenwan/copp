...
fn main() -> Result<(), CoppError> {
    ...
    // 5) Post-process COPP3-SOCP profile: (profile,s) -> t(s) -> s(t)
    ...
    // profile_qp1 is a feasible but possibly suboptimal profile for COPP3. It can be directly used for control or as a reference for further optimization unless a more optimal profile is required.
    ...
    // 8) Post-process COPP3-SOCP profile: (profile,s) -> t(s) -> s(t)
    // t_final is the traversal time of the path.
    // t_s[i] is the time at which the path parameter s_i is reached.
    ...
    // profile_qp2 is a less conservative and more optimal profile for COPP3 compared with profile_qp1.
