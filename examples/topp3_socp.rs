...
fn main() -> Result<(), CoppError> {
    ...
    // The following code is optional. It substitutes the 1st-order constraint `a[k]<=amax[k]` with `a[k]<=a_ra0[k]` to get a less conservative TOPP3-SOCP result. The user can skip this step and directly use `amax` for TOPP3-SOCP.
    ...
    // 7) Solve TOPP3-SOCP with the linearization profile from TOPP3-SOCP.
    let profile_qp2 = {
