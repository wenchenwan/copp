/**
 * @file test_topp3_socp.c
 * @brief End-to-end smoke test for TOPP3-SOCP.
 */
...
int main(void)
{
    ...
    status = topp3_socp(problem, socp_options, &profile);
    if (expect_ok(status, "topp3_socp"))
    {
        copp_vec_f64_free(a_seed);
        copp_robot_free(robot);
        copp_path_free(path);
    ...
    status = topp3_socp_expert(problem, socp_options, &expert_result);
    if (expect_ok(status, "topp3_socp_expert"))
    {
    ...
    assert(expert_result.objective_terms.len == 0);
    assert(expert_result.solver_status == COPP_CLARABEL_SOLVER_STATUS_SOLVED ||
           expert_result.solver_status == COPP_CLARABEL_SOLVER_STATUS_ALMOST_SOLVED);
    copp3_socp_result_free(expert_result);

    printf("TOPP3-SOCP C smoke test passed: t_final=%.17g, len=%zu\n",
           t_final,
           profile.a.len);
